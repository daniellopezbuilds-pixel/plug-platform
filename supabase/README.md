...
| Production | **Applied through `20260922220000`** — verified against `schema_migrations` on 2026-09-23. Somebody pushed the sixteen CSLB-to-name-matching files and left production **linked**; found that way and relinked to staging. `20260923120000` and `20260923130000` are not on production yet |
| Staging | **Fully up to date** — everything through `20260923130000` applied |
| CSLB licence data | **Staging: imported on 2026-09-22**, 244,519 rows as of the 2026-09-19 file, all classifications, `c10_count` 29,123. Verified afterwards: licence 1117700 returns `wrong_classification` with `{C36}`. **Production: not checked from here.** The CSLB migrations are applied there now, so an empty import table answers `not_found` for every licence until the file is imported. It is data, not schema, and does not travel on a `db push` — see "Importing the CSLB file" |
...
| `20260921120000_cslb_license_verification.sql` | Applied to staging 2026-09-21. On production — found applied 2026-09-23 | CSLB C-10 licence verification: `cslb_licenses`, its staging table and the `cslb_imports` ledger, the decision table in `cslb_evaluate_license()`, and an AFTER trigger on `role_credentials` that re-checks a licence whenever it is written. Adds `user_badge_reviews.reason` and a `needs_review` decision. **Inert until `20260921130000`** — every write path early-outs while the `license_verified` badge is inactive, which also stops it breaking C-10 signups on a database with no imported data |
| `20260921130000_activate_license_verified.sql` | Applied to staging 2026-09-21. On production — found applied 2026-09-23 | Switches `license_verified` active and sweeps every existing C-10 against the import. **Push only after the import has been loaded into that project** — it warns and skips the sweep rather than running it against data that is missing or stale. Does not touch `business_verified` or the `employer_verified` re-sync from section 7b of `20260916130000` |
| `20260921140000_cslb_ownership_checks.sql` | Applied to staging 2026-09-21. On production — found applied 2026-09-23 | Two ownership checks added to `cslb_evaluate_license()`: `already_claimed` (the number is verified on another account) and `name_mismatch` (the account's business name does not resemble the CSLB record). CSLB publishes every licence number publicly, so the original seven checks proved a number was real and nothing about who typed it. Adds a **unique partial index** making one-licence-one-verified-badge a database invariant, demoting any duplicate already present before creating it. Changes the signatures of `cslb_evaluate_license` and `cslb_apply_check`, so both are dropped and recreated |
| `20260921150000_licence_notifications.sql` | Applied to staging 2026-09-21. On production — found applied 2026-09-23 | Tells people what happened. Adds `user_badges.check_reason` (the reason, denormalised so the badge OWNER can read it — `user_badge_reviews` is admin-only because it carries private notes and the other party in a claim dispute), the `email_outbox` queue, a notification and a queued security email to the licence holder when a claim is blocked, and a trigger notifying the contractor on verified / rejected / revoked. Backfills `check_reason` for badges `20260921140000` already queued |
| `20260921160000_claim_alert_dedupe_fix.sql` | Applied to staging 2026-09-21. On production — found applied 2026-09-23 | Fixes the held-back-claim alert never firing. `20260921150000` deduped on whether an `already_claimed` **review row** existed in the last 7 days, but a review row and an alert are different events — every row written before alerting shipped suppressed the first real alert. Dedupe now reads the `license_claim_attempt` **notification** instead, moved into `cslb_raise_claim_alert()` so the backfill shares one copy. Backfills the alerts already swallowed, indexes `notifications (user_id, type, created_at)`, and rewrites both review-badge descriptions so they read correctly in every status |
| `20260922120000_cslb_all_classifications.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | The import stops filtering to C-10, so a licence in another classification can be answered `wrong_classification` instead of `not_found`. Found by licence 1117700 — a real, current, CLEAR **C-36** licence whose holder was told we could not find their number. `wrong_classification` had existed since `20260921120000` and could never fire. Three parts: the decision table checks classification **before** status and expiry (a no-op for every row in the table today, since all of them hold C10 — and what lets non-C-10 rows be stored **slim**: number, classifications and `class_keys`, no names or cities for 215,396 contractors who will never hold an account); `cslb_imports.c10_count` plus a second guard in `cslb_commit_import()` on the C-10 count alone; and refreshed table comments. **No table DDL beyond the new ledger column** — `class_keys` was stored rather than assumed from the start, exactly so the filter could widen without touching a function. `cslb_commit_import` is dropped and recreated because its returns table grows, so its grants are restated |
| `20260922130000_cslb_surface_classification.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Says WHICH classification. `20260922120000` made `wrong_classification` reachable but it stopped at "not a C-10", so the reviewer's next move was to look up a number we had already looked up. `cslb_evaluate_license()` now returns `checked_classifications text[]` (dropped and recreated — the returns table grows), `cslb_apply_check()` writes it onto the audit row, `user_badge_reviews.checked_classifications` stores it, and the admin card renders it labelled from `CSLB_CLASSIFICATIONS` in `lib/cslb.tsx`. **The decision table is untouched** — same ten rules, same order, one more output column. Populated on every path that read a record, not only `wrong_classification`; NULL for `no_number`, `stale_data` and `not_found`, which read none |
| `20260922140000_backfill_review_classifications.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Fills `checked_classifications` on audit rows written before the column existed. **A re-sweep cannot do this** — `cslb_apply_check()` returns `unchanged` and writes nothing when status, expiry and reason all match, which is the guard that stops a standing conflict re-alerting weekly, so a column added after a check ran is never filled by running the check again. Backfills only rows whose reason actually read a record (not `no_number`/`stale_data`/`not_found`) and whose `source_as_of` matches the current import, so no audit row is given evidence from a file it never consulted. Idempotent. Two rows on staging; none on production if `20260922130000` lands before its first import, which is the intended order |
| `20260922150000_badge_marker_and_tab_links.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Two UI-serving changes. `public_badges` gains `icon` and `label` (appended, so the five existing columns keep their positions), letting the single marker beside a name be drawn as the badge's own glyph and titled with what it verified — from the one batched query `useProfileBadges` already sends, rather than a second round trip. And the licence notifications deep-link into the now-tabbed profile editor: `license_verified` to `?tab=badges`, `license_rejected` / `license_revoked` / `license_claim_attempt` to `?tab=credentials`. `cslb_raise_claim_alert()` and `notify_license_decision()` are replaced whole with **only the link changed** — verified by diffing both against their originals. **Existing `notifications.link` rows are not rewritten**: a link records where a message pointed when it was sent, and an old one still lands on a working page |
| `20260922160000_realtime_admin_queues.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Adds `employer_documents`, `general_requests`, `sponsored_listings`, `user_badges` and `profiles` to the `supabase_realtime` publication, so the admin panel's tab counts update without a refresh. Postgres only streams what a publication carries — a subscription to a table outside one succeeds and then never fires, which is the failure this prevents. **Does not widen access:** `postgres_changes` applies RLS per subscriber, so publication membership enables push and nothing else. `profiles` is the one worth a second thought (any authenticated user can already read it, and can now watch it) and is included because the union queue is a query over profiles — without it that one count would sit stale while the other four moved. Idempotent, guarded on `pg_publication_tables`; leaves replica identity at the default |
| `20260922170000_job_detail_columns.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Thirteen columns on `jobs` so a listing can be judged without applying: `classification`, `work_type`, `starts_on`, `duration`, `shift`, `pay_rate_min`/`max`/`pay_unit`, `min_years_experience`, `certification_required`, `requires_own_tools`, `requires_own_transport`. Text plus CHECK throughout, no enums. **No `city` column** — `jobs.location` is reused and the form binds it to the same `LocationField` as signup and the profile, because two columns answering "where" would disagree the first time one was edited. **`pay` is kept, not dropped**: the seven pre-existing rows have their rate only there, and `formatPay()` falls back to it. **Requiredness is in the form, not the schema** (`jobDraftErrors()` in `lib/jobs.tsx`) — a NOT NULL would apply to those seven legacy rows too. `classification` and `min_years_experience` reuse the exact strings from `lib/signupRoles.tsx`, so a job's requirement and a worker's profile are one vocabulary. Adds `jobs_created_at_idx`; filter indexes deliberately deferred until something filters |
| `20260922180000_application_loop.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Closes the application loop. **In-app notifications already existed** — `on_new_application` and `on_status_change` are in the baseline and fire — so this replaces the two functions rather than adding triggers: `notify_new_applicant()` now names the applicant (falling back to "Someone") and `notify_status_change()` fires on **accepted/rejected only**, where it used to announce a move back to pending as if it were a decision. Both now queue `email_outbox` rows in the same transaction as the notification, so an application cannot produce one without the other. Adds `conversations.job_id` (FK to `jobs`, **ON DELETE SET NULL** — taking a job down must not delete the correspondence) plus a partial index. The job context is a column and a pinned banner, **not a first message**: the `messages` INSERT policy ends in `NOT is_messaging_blocked()`, which refuses an unsubscribed worker sending to an employer — exactly the electrician clicking Message on their own application |
| `20260922190000_resume_access_and_classification.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | **Narrows who can read a resume.** "Eligible users can view resumes" used `can_message()`, which is true for an accepted **connection** — a directory handshake that granted permanent access to somebody's address and work history. Replaced with `can_view_resume(owner, viewer)`: the owner, or an employer who received an application from them, and directional so an applicant cannot read an employer's. `can_message()` is left alone because it is also the WITH CHECK on `conversation_participants`, where the connections branch is correct. Also adds `profiles.classification` + CHECK + backfill from `role_credentials` and carries it through `handle_new_user()` — an employer could not see an applicant's classification at all, because `role_credentials` is owner-and-admin only. Same reasoning and same pattern as `years_experience` |
| `20260922200000_realtime_notifications.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Adds `notifications` to `supabase_realtime`; `20260922160000` covered the admin-queue tables and not this one. The bell had subscribed to it since long before — the subscription succeeded and silently never fired, which looked exactly like a bell that only updates on reload. RLS still scopes rows per subscriber. Adds a partial `notifications (user_id) where read = false` for the unread counts |
| `20260922210000_name_match_squash_whitespace.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Fixes a false `name_mismatch` on names typed without spaces — "Thomasheinzmanelectrical" vs CSLB's "THOMAS HEINZMAN ELECTRICAL". A run-together name is one token and can never be a subset of a multi-word token set, so **every** multi-word business name failed this way. Adds `cslb_squash_name()` and a second comparison in `cslb_names_match()` on the normalised string with whitespace removed — squashed BEFORE the stop list, or the trade word is dropped from one side only and the two never line up. Exact equality, not fuzzy. Measured on the 29,123 C-10 names: fixes all 28,804 that would fail; newly collapses 215 groups / 802 licences, of which 156 are initials and the rest spacing variants of identical names. Clears `check_reason` on queued `name_mismatch` badges so the next sweep re-decides them — `cslb_apply_check()`'s `unchanged` early return would otherwise never re-evaluate them. **Superseded in part by `20260922220000`** |
| `20260922220000_name_match_null_guard_fix.sql` | Applied to staging 2026-09-22. On production — found applied 2026-09-23 | Fixes a regression in `20260922210000`, caught by that migration's own test before it left staging. Its empty-name guard used `nullif(squash(claimed), '')`, so a blank name made the comparison NULL rather than false; `false or NULL` is NULL, and `not NULL` in `cslb_evaluate_license()` is not true — so the `name_mismatch` branch never fired and **an account with an empty or punctuation-only business name auto-verified against any otherwise-clean licence number**. Replaced with an explicit `<> ''` test so every operand is a plain boolean. Production takes both files in one push and never sees the intermediate state |
| `20260923120000_realtime_messages.sql` | Applied to staging 2026-09-23. **Not on production** | Adds `messages` and `conversation_participants` to `supabase_realtime`, for the sidebar Messages badge. Checked on both projects first: `messages` was published on **production only** (added in the dashboard, never in a migration) and `conversation_participants` on **neither** — so on staging the badge never rose, and on both it never cleared when a thread was read, because the `last_read_at` update it recounts on never streamed. Idempotent; on production `messages` is left alone. RLS still scopes rows per subscriber |
| `20260923130000_ad_insert_admin_only.sql` | Applied to staging 2026-09-23. **Not on production** | **Closes a forged-payment hole.** The `sponsored_listings` INSERT policy was `is_admin() OR (submitted_by = auth.uid() AND status = 'pending')`, with every other column unchecked — a demo electrician inserted a row claiming `source 'brand'`, `payment_status 'paid'`, six months, and it appeared in the admin Advertisement requests queue as prepaid with no payment. Now **admins only**. Nothing legitimate used the user branch: brand checkout inserts and the webhook marks paid with the service role (RLS bypassed), admin house ads use `is_admin()`, and the one user-side caller — the free ad request on /dashboard/requests — was removed the same day. Not narrowed to "own unpaid brand row" on purpose: such a row skips the checkout route's validation (the `javascript:` link guard, title, city) and the resume-payment path would then take payment for it. Verified on staging against the running app: forged and direct inserts refused (42501), admin create works, checkout creates its unpaid row, a signed webhook marks it paid, a badly signed one is refused (400). SELECT, UPDATE, DELETE unchanged; no data touched |
